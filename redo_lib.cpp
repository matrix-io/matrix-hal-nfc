...
const uint8_t GI[] = {
    0x46, 0x66, 0x6D, 0x01,         0x01, 0x10, /*VERSION*/ 0x03,
    0x02, 0x00, 0x01, /*WKS*/ 0x04, 0x01, 0xF1 /*LTO*/
};
...
static phStatus_t LoadProfile(nfc_data *nfcData) {
  ...
  /*
   * These lines are added just to SIGSEG fault when non 14443-3 card is
   * detected
   */
  /*
  ...
  */
   ...
   */
  status = phacDiscLoop_SetConfig(&(nfcData->sDiscLoop),
  ...
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_PAS_LIS_TECH_CFG, PH_OFF);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_ACT_LIS_TECH_CFG, PH_OFF);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_ACT_POLL_TECH_CFG, PH_OFF);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(&(nfcData->sDiscLoop),
                                  PHAC_DISCLOOP_CONFIG_ENABLE_LPCD, PH_OFF);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_COLLISION_PENDING, PH_OFF);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(&(nfcData->sDiscLoop),
                                  PHAC_DISCLOOP_CONFIG_ANTI_COLL, PH_ON);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_TYPEA_DEVICE_LIMIT, PH_ON);
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phacDiscLoop_SetConfig(
      &(nfcData->sDiscLoop), PHAC_DISCLOOP_CONFIG_OPE_MODE, RD_LIB_MODE_NFC);
  PH_CHECK_SUCCESS(status);
...
phStatus_t NfcRdLibInit(nfc_data *nfcData) {
   ...
   */
  status = phbalReg_Stub_Init(&(nfcData->sBalReader),
  ...
  status =
      phbalReg_SetConfig(&(nfcData->sBalReader), PHBAL_REG_CONFIG_HAL_HW_TYPE,
  ...
  status = phhalHw_Nfc_IC_Init(
      &(nfcData->sHal_Nfc_Ic), sizeof(phhalHw_Nfc_Ic_DataParams_t),
      &(nfcData->sBalReader), 0, nfcData->bHalBufferTx,
      sizeof(nfcData->bHalBufferTx), nfcData->bHalBufferRx,
      sizeof(nfcData->bHalBufferRx));
  PH_CHECK_SUCCESS(status);
  ...
  /*
   * Initializing specific objects for the communication with MIFARE (R) Classic
   * cards. The MIFARE (R) Classic card is compliant of ISO 14443-3 and ISO
   * 14443-4
   */
   ...
   */
  status = phpalI14443p3a_Sw_Init(&(nfcData->spalI14443p3a),
                                  sizeof(phpalI14443p3a_Sw_DataParams_t),
                                  &(nfcData->sHal_Nfc_Ic.sHal));
  PH_CHECK_SUCCESS(status);

  /*
   * Initialize the I14443-A PAL component
   */
  status = phpalI14443p4a_Sw_Init(&(nfcData->spalI14443p4a),
   ...
   */
  status = phpalI14443p4_Sw_Init(&(nfcData->spalI14443p4),
   ...
   */
  status = phpalI14443p3b_Sw_Init(&(nfcData->spalI14443p3b),
                                  sizeof(phpalI14443p3b_Sw_DataParams_t),
                                  &(nfcData->sHal_Nfc_Ic.sHal));
  PH_CHECK_SUCCESS(status);
   ...
   */
  status = phpalMifare_Sw_Init(&(nfcData->spalMifare),
                               sizeof(phpalMifare_Sw_DataParams_t),
                               &(nfcData->sHal_Nfc_Ic.sHal), NULL);
  PH_CHECK_SUCCESS(status);

  /*
   * Initialize the discover component
   */
  status = phacDiscLoop_Sw_Init(&(nfcData->sDiscLoop),
  ...
  status = phalMfc_Sw_Init(&(nfcData->salMfc), sizeof(phalMfc_Sw_DataParams_t),
                           &(nfcData->spalMifare), NULL);
  PH_CHECK_SUCCESS(status);
  ...
  status = phhalHw_Rc523_ReadRegister(&(nfcData->sHal_Nfc_Ic.sHal),
                                      PHHAL_HW_RC523_REG_VERSION, &icVersion);
#endif
#if defined NXPBUILD__PHHAL_HW_RC663
  status = phhalHw_Rc663_ReadRegister(&(nfcData->sHal_Nfc_Ic.sHal),
                                      PHHAL_HW_RC663_REG_VERSION, &icVersion);
#endif
  PH_CHECK_SUCCESS(status);
...
static PyObject *Mifare_new(PyTypeObject *type, PyObject *args,
                            PyObject *kwds) {
...
PyObject *Mifare_select(Mifare *self) {
   ...
   */
  status =
      phacDiscLoop_SetConfig(pDiscLoop, PHAC_DISCLOOP_CONFIG_NEXT_POLL_STATE,
  ...
  if ((status & PH_ERR_MASK) != PHAC_DISCLOOP_DEVICE_ACTIVATED) {
    if (handle_error(status, SelectError)) {
      ...
      return PyErr_Format(SelectError, "DiscLoop_Run command failed: %02X",
                          (status & PH_ERR_MASK));
    }
  }
   ...
   */
  status = phacDiscLoop_GetConfig(pDiscLoop, PHAC_DISCLOOP_CONFIG_TECH_DETECTED,
                                  &wTagsDetected);
  if (handle_error(status, SelectError)) return NULL;
  ...
  if (PHAC_DISCLOOP_CHECK_ANDMASK(wTagsDetected,
                                  PHAC_DISCLOOP_POS_BIT_MASK_A)) {
    phacDiscLoop_Sw_TypeA_Info_t *pTypeATargetInfo =
        &(pDiscLoop->sTypeATargetInfo);
    ...
    for (uint8_t i = 0; i < byteBufferSize; i++) {
      sprintf(&asciiBuffer[2 * i], "%02X",
              pTypeATargetInfo->aTypeA_I3P3[0].aUid[i]);
    }

    return PyUnicode_FromString(asciiBuffer);

  } else {
    return PyErr_Format(SelectError, "DISCLOOP_CHECK_ANDMASK failed: %02X",
                        (status & PH_ERR_MASK));
  }
...
PyObject *Mifare_read_block(Mifare *self, PyObject *args) {
  ...
  if (handle_error(status, ReadError)) return NULL;
    ...
    // The read command reads a full 16 bytes, but we only want to return 1 page
    // worth of data for this command.
...
PyObject *Mifare_write_block(Mifare *self, PyObject *args) {
  ...
  if (dataLen != WRITE_BUFFER_LEN) {
    return PyErr_Format(WriteError, "Write data MUST be specified as %d bytes",
                        WRITE_BUFFER_LEN);
  }
...
PyObject *Mifare_get_version(Mifare *self) {
  ...
  return Py_BuildValue("{s:B, s:B, s:B, s:B, s:B, s:B, s:B}", "vendor\0",
                       version[1], "tag_type\0", version[2], "tag_subtype\0",
                       version[3], "version_major\0", version[4],
                       "version_minor\0", version[5], "tag_size\0", version[6],
...
uint8_t *_Mifare_read(Mifare *self, uint16_t *bytesRead) {
  ...
  uint16_t userMemorySizeBytes =
      memorySize *
      8;  // According to NFC Forum Type 2 Tag Operation Specification

  uint8_t remainder = userMemorySizeBytes % MFC_BLOCK_DATA_SIZE;

  // Round up to the neared MFC_BLOCK_DATA_SIZE so we have enough buffer space
  // to read to the end.
  if (remainder != 0) {
  ...
  for (uint16_t pageIdx = 0; pageIdx < userMemorySizePages; pageIdx++) {
    ...
    status = phalMful_Read(&(self->nfcData.salMfc), pageToRead,
...
PyMethodDef Mifare_methods[] = {
    {"select", (PyCFunction)Mifare_select, METH_NOARGS,
     "Select a Mifare card if present. Returns the card UID"},
    {"read_block", (PyCFunction)Mifare_read_block, METH_VARARGS,
     "Read 4 bytes starting at the specified block."},
    {"read_sign", (PyCFunction)Mifare_read_sign, METH_NOARGS,
     "Read 32 bytes card manufacturer signature."},
    {"write_block", (PyCFunction)Mifare_write_block, METH_VARARGS,
     "Write 4 bytes starting at the specified block."},
    {"get_version", (PyCFunction)Mifare_get_version, METH_NOARGS,
     "Read version data as a dict."},
    {"clear_block", (PyCFunction)Mifare_clear_block, METH_VARARGS,
     "Clear 4 bytes starting at the specifed block."},
    {"read", (PyCFunction)Mifare_read, METH_VARARGS,
     "Reads all user data stored on the selected tag. Currently supports "
     "NTAG213/215/216."},
    {"read_ndef", (PyCFunction)Mifare_read_ndef, METH_VARARGS,
     "Attempt to read the first NDEF Message stored on the selected tag."},
    {"pwd_auth", (PyCFunction)Mifare_pwd_auth, METH_VARARGS,
     "Authenticate with password"},
    {NULL} /* Sentinel */
};

PyTypeObject MifareType = {
    PyVarObject_HEAD_INIT(NULL, 0) "nxppy.Mifare", /* tp_name */
